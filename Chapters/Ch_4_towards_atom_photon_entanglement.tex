...
A non-polarizing beamsplitter can be described as an operator which transforms the annihilation (and creation) operators at the input ports to those at the output ports according to
\begin{equation}
\left(\begin{array}{l}
    a_{c} \\
    a_{d}
...
\begin{equation}
\begin{aligned}
&a_{c}=\frac{1}{\sqrt{2}}\left(a_{a}+i a_{1}\right) \\
&a_{d}=\frac{1}{\sqrt{2}}\left(i a_{b}+a_{1}\right)
\end{aligned}
\end{equation}
and can be used, for example, to derive the famous Hong-Ou-Mandel effect\cite{Hong1987}.
