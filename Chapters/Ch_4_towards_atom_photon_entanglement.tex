...
\end{equation}
and $\hat{a}$ is the annihilation operator. Evaluating Eq. (\cite{eq:g2long}) for a single mode field thus reduces to
...
\end{aligned}
\end{equation}
It is now trivial to show that, as expected, a single photon state impinging on port $a$ will result in a photon detected at either output port with equal probability. For the input state $\ket{a}_a\ket{0}_b$, we have:
\begin{equation}
    \begin{aligned}
